...
<body>
    <div class="header clearfix">
        <div class="col-l">
            ...
            <p class="muted small">Customer statement</p>
    ...
    <div class="panel">
        <p class="small muted">Account</p>
        <p style="font-weight: 500;">
            <span class="mono muted">{{ $customer->code }}</span> &middot; {{ $customer->name }}
        </p>
        @if ($customer->phone)
            <p class="small muted mono">{{ $customer->phone }}</p>
        @endif
        @if ($customer->address)
            <p class="small muted">{{ $customer->address }}</p>
        @endif
    </div>
    ...
    <table class="tiles">
        <tr>
            <td>
                <p class="tile-label">Opening balance</p>
                <p class="tile-value mono">{{ number_format($opening, 2) }}</p>
            </td>
            <td>
                <p class="tile-label">Period debit</p>
                <p class="tile-value mono neg">{{ number_format($totalDebit, 2) }}</p>
            </td>
            <td>
                <p class="tile-label">Period credit</p>
                <p class="tile-value mono pos">{{ number_format($totalCredit, 2) }}</p>
            </td>
            <td>
                <p class="tile-label">Closing balance</p>
                <p class="tile-value mono {{ $closing > 0 ? 'neg' : ($closing < 0 ? 'pos' : '') }}">
    ...
    <table>
        ...
        <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if ($e['reference'])
                            ...
                            @if ($e['description'])
                                <span class="small muted"> &middot; {{ $e['description'] }}</span>
                            @endif
                        @else
                    ...
                    <td class="right mono pos">
                        {{ $e['credit'] > 0 ? number_format($e['credit'], 2) : '' }}
                        @if ($e['credit'] <= 0) <span class="muted">—</span> @endif
                    </td>
            ...
            <tr class="row-closing">
                ...
                <td class="right mono {{ $closing > 0 ? 'neg' : ($closing < 0 ? 'pos' : '') }}">
                    {{ number_format($closing, 2) }}
                </td>
            </tr>
        </tbody>
    ...
    <p class="small muted" style="margin-top: 12px;">
        All amounts in {{ $business['currency'] ?? '' }}. Positive balance = customer owes the business.
        Negative balance = customer has credit on account.
    </p>
